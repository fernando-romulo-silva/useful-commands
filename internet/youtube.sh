...
youtube-dl --write-sub --skip-download --sub-lang=en "https://www.youtube.com/watch?v=UZ9VKxxGo58"
...
youtube-dl -f "bestvideo[height<=480][ext=mp4]+bestaudio/[height <=? 480]" --playlist-reverse -cio '%(autonumber)s - %(title)s.%(ext)s' --restrict-filenames https://www.youtube.com/playlist?list=YOUR LIST HERE --sub-lang en --convert-subs srt --write-sub